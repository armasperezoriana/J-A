function ajaxRegistrar(datos){
  $.ajax({
            type: "POST",
            url: "?url=nomina&opcion=registrar",
            data: datos,
            beforeSend: function(){
              $('#imagen').show();
...
function ajaxModificar(datos){
    $.ajax({
      type: "POST",
      url: "?url=nomina&opcion=modificar",
      data: datos,
      beforeSend: function(){
        $('#imagen').show();
        $('.mensajes').html('Procesando datos...');
...
$(".formulario_registrar").on('submit', function(e){
    ...
    if (n > 0) {
     ...
     if (validarFechas(datos)&&expresion_horas_extras(datos) &&expresion_fecha_pago(datos)) {
          ajaxRegistrar(datos);
      }
    }
})
...
function eliminar(id){
              Swal.fire({
                title: '¿Deseas realmente eliminar éste elemento?',
                ...
                if (result.isConfirmed) {
                  $.ajax({
                    type: "POST",
                    url: "?url=nomina&opcion=eliminar",
                    data: 'id=' + btoa(id),
                    beforeSend: function(){
                    },
                    success: function(respuesta){
